...
def fetch_ibov(url: str) -> pd.DataFrame:
    r = requests.get(url, timeout=30)
    r.raise_for_status()
    data = r.json()
    df = pd.DataFrame(data.get("results", []))

    # Corrige data de referência
...
def load_to_bq_append(df: pd.DataFrame, table_id: str) -> dict:
    if df.empty:
        return {"rows": 0}
    ...
    if USE_STREAMING:
        ...
        job = client.load_table_from_dataframe(
            df, table_id, job_config=bigquery.LoadJobConfig(write_disposition="WRITE_APPEND")
        )
        job.result()
        dest = client.get_table(table_id)
        return {"rows": int(len(df)), "table_rows": int(dest.num_rows)}
